...
use crate::token_store::{OAuthToken, TokenStore}; // Updated import
...
impl OAuthConfig {
    ...
    pub fn gemini() -> Result<Self> { // Changed return type to Result<Self>
        let client_id = std::env::var("GEMINI_CLIENT_ID")
            .context("Environment variable GEMINI_CLIENT_ID not set for Gemini OAuth")?;
        let client_secret = std::env::var("GEMINI_CLIENT_SECRET")
            .context("Environment variable GEMINI_CLIENT_SECRET not set for Gemini OAuth")?;
